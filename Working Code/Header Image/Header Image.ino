/*******************************************************************************
 * LVGL Widgets
 * English Alphabet Tabs With an Image
 * Loaded from header file, in C array
 ******************************************************************************/
#include <lvgl.h>
...
void setup() {
    Serial.begin(115200);
    Serial.println("LVGL English Alphabet Tabs + Image");
    ...
    if (!disp_draw_buf) {
        ...
        void add_image = [](lv_obj_t *parent) {
            // Check if the image structure is available (it is if 'tree.h' was included)
            if (&tree == NULL) {
                Serial.println("Error: Image descriptor 'logo_img' not found. Did you include the header?");
                return;
            }
            ...
            lv_obj_center(img); 
            Serial.println("Image loaded successfully from Flash memory (C Array).");
        };

        // English letters for each tab
